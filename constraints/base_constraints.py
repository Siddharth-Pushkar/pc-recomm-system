...
def add_base_constraints(model, cpu_vars, gpu_vars, mobo_vars, ram_vars,
                         storage_vars, psu_vars, cooler_vars):
    model.Add(sum(cpu_vars)     == 1)
    model.Add(sum(gpu_vars)     == 1)
    model.Add(sum(mobo_vars)    == 1)
    model.Add(sum(ram_vars)     == 1)
    model.Add(sum(storage_vars) == 1)
    model.Add(sum(psu_vars)     == 1)
    model.Add(sum(cooler_vars)  == 1)
    return model
...
def add_budget_constraint(model,
    ...
    total_cost = (
        ...
        sum(int(storage_df.loc[i, "price"]) * storage_vars[i] for i in range(len(storage_df))) +
        sum(int(psu_df.loc[i,     "price"]) * psu_vars[i]     for i in range(len(psu_df)))     +
        sum(int(cooler_df.loc[i,  "price"]) * cooler_vars[i]  for i in range(len(cooler_df)))
    )
